...
const LoginPage: React.FC = () => {
  ...
  const onSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Small delay to ensure AuthContext state and localStorage are flushed
      // before mounting protected admin pages (avoids a race that can cause 401)
      setTimeout(() => {
        try { console.debug('Navigating after login, user role:', userData.role, 'tokenExists:', !!localStorage.getItem('token')) } catch (e) {}
            ...
            <form onSubmit={onSubmit} aria-label="Login form">
              ...
              <div className="mt-6 flex justify-center">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="bg-[#6b3f2f] hover:bg-[#5a3426] disabled:bg-gray-400 disabled:cursor-not-allowed text-white px-8 py-2 rounded-md shadow-md transform transition-transform hover:scale-105 w-full md:w-auto"
                >
              ...
              {errors.submit && <div className="mt-4 text-rose-600 text-center">{errors.submit}</div>}
            </form>
          </div>
        </section>
