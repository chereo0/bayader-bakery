...
interface MessageItem {
  ...
  timestamp?: string
  read: boolean
  type: 'system' | 'admin' | 'staff' | 'driver'
  createdAt: string
}
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <div className={`mt-0.5 ${selectedMessage?._id === message._id ? 'text-white' : ''}`}>
                        {getMessageIcon(message.from.role as any)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <p className={`font-medium text-sm truncate ${selectedMessage?._id === message._id ? 'text-white' : 'text-gray-900'}`}>
                            {message.from.name}
                            <span className={`ml-2 text-xs ${selectedMessage?._id === message._id ? 'text-white/70' : 'text-gray-500'}`}>
                              ({message.from.role})
                            </span>
                          </p>
              ...
              <div className="flex items-center justify-between mb-4 pb-4 border-b">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2 text-sm text-gray-600">
                    ...
                    <span>•</span>
                    <span>{new Date(selectedMessage.createdAt).toLocaleDateString()} {selectedMessage.timestamp}</span>
                  </div>
                </div>
              </div>
