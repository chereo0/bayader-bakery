...
const UserTable: React.FC<Props> = ({ users, onEdit, onDelete, onBulkDelete, isLoading }) => {
    ...
    <div className="bg-white p-4 rounded shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody>
            {users.map((u, idx) => (
              <tr key={String(u.id)} className={`border-t ${idx % 2 === 0 ? 'bg-white' : 'bg-neutral-50'} hover:bg-neutral-100 transition`}>
                ...
                <td className="py-3 flex gap-2">
                  <button onClick={()=>onEdit(u.id)} className="px-3 py-1 bg-[#6b3f2f] text-white rounded" disabled={isLoading}>Edit</button>
                  {onDelete && (
                    <button onClick={()=>onDelete(u.id)} className="px-3 py-1 bg-red-600 text-white rounded" disabled={isLoading}>Delete</button>
                  )}
                </td>
              </tr>
            ))}
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="text-sm text-[#6b4f45]">
          Bulk Actions: 
          <button
            className={`ml-3 px-3 py-1 rounded ${selected.length ? 'bg-red-600 text-white' : 'bg-[#f0e6de]'}`}
            onClick={() => onBulkDelete && onBulkDelete(selected)}
